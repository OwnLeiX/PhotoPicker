package lx.photopicker.ui.widget;
...
public class OwnStateLayout extends FrameLayout implements View.OnClickListener
{
	...
	private OwnStateLayout(Builder builder)
	{
		super(builder.mContext);
		mEmptyView = builder.empty;
		mSuccessView = builder.success;
		mFailedView = builder.failed;
		mLoadingView = builder.loading;
		mOnRetryListener = builder.onRetryListener;
		mRetryResId = builder.retryResId;
		init();
	}
	...
	private void initStateViews()
	{
		...
		} else
		{
			...
			if (mRetryResId != -1)
			{
				View retryView = mFailedView.findViewById(mRetryResId);
				if (retryView != null)
	...
	public static class Builder
	{
		...
		{
			OwnStateLayout instance = new OwnStateLayout(this);
			return instance;
		}
