...
import lx.photopicker.task.ScanTask;
import lx.photopicker.task.SizeClipCallback;
import lx.photopicker.task.SizeClipTask;
...
public class PickerManager implements ScanCallback, SizeClipCallback
{
	...
	public void scan(Context context, ScanCallback callback)
	{
		...
		mTask = new ScanTask(context, this);
		Pool.execute(mTask);
	}
	...
	public boolean doCallback(DoCallbackCallback callbackCallback)
	{
		...
		if (mPickerCallback != null)
		{
			...
			{
				Pool.execute(new SizeClipTask(mPickedPhotos, mCachedDir, this));//修正所有选中图片的尺寸
				return false;
			} else
			{
